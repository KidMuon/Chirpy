...
const addChirpyRedByID = `-- name: AddChirpyRedByID :one
UPDATE users
...
func (q *Queries) AddChirpyRedByID(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, addChirpyRedByID, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
